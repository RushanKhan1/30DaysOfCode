...
int minimum_index(vector<int> seq) {
  if (seq.empty()) { //checking if the vector is empty
    ...
    return min_idx; //returning the index of the minimum element in the array.
}

class TestDataEmptyArray { //class made for testing
public:
    static vector<int> get_array() {
      vector<int>emptyarray{} ; //declaring an empty array
      return emptyarray;
    }
...
public:
    static vector<int> get_array() { 
      vector<int> twominimums;
      twominimums = { 1, 2, 3, 4, 1};
      return twominimums;
    }
...
void TestWithEmptyArray() { //constructor of the class
    try {
